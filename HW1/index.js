...
const max = Math.max(nokia, apple, samsung)
const min = Math.min(nokia, apple, samsung)
const sumProducts = nokia + apple + samsung
const sumProductsFloor = Math.floor(nokia) + Math.floor(apple) + Math.floor(samsung)
...
document.write(`nokia = ${nokia}<br>`, `apple = ${apple}<br>`, `samsung = ${samsung}<br>`,)
...
document.write(`Остаток с 500 $ без округлений: 500 - ${sumProducts} = ${rest}$ <br>`)
document.write(`Среднее значение цен округленое до 2 знаков после запятой:  ${averageSumToFix}$ <br>`)
document.write(`Случайная скидка на товар:  ${discount} % <br>`)
document.write(`Сумма товара с учетом скидки округленое до 2 знаков после запятой:  ${order}$ <br>`)
document.write(`Чистая прибыль с учетом скидки: ${profit} $ <br>`)